...
vi.mock('lucide-react', () => {
  const mockIcon = (name: string) => function MockIcon() {
...
vi.mock('../Card', () => ({
  default: (props) => (
    <div data-testid={`card-${props.card.id}`}>
      {props.card.content}
    </div>
  ),
}));
...
vi.mock('@hello-pangea/dnd', () => {
  return {
    ...
    Droppable: (props: any) => props.children({
      draggableProps: {},
      innerRef: () => {},
      placeholder: null,
...
interface ParticipantsPanelProps {
  isOpen: boolean;
  onClose: () => void;
  participants: Array<{id: string; name: string; color: string; boardId: string; lastOnline: number}>;
...
const mockParticipantsPanel = (props: ParticipantsPanelProps) => {
  ...
  return (
    <div data-testid="participants-panel">
      <h2>Participants</h2>
      <button onClick={props.onClose} aria-label="close panel">Close</button>
...
vi.mock('../Board', async (importOriginal) => {
  ...
  const MockedBoard = (props) => {
    ...
    React.useEffect(() => {
      ...
      presenceService.setupPresence(props.match?.params?.boardId || 'test-board-id', 'Current User');
...
describe('Board Component - Participants Integration', () => {
  ...
  it('allows the current user to edit their name', async () => {
    ...
    expect(boardService.updateParticipantName).toHaveBeenCalledWith('current-user-id', 'New Username');
    expect(presenceService.updateParticipantName).toHaveBeenCalledWith('current-user-id', mockBoardId, 'New Username');
  });
  ...
  it('shows empty state when there are no participants', async () => {
    ...
    vi.mocked(presenceService.subscribeToParticipants).mockImplementationOnce((boardId, callback) => {
      callback([]);
      return vi.fn();
    });
