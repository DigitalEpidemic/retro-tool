...
describe("ShareModal", () => {
  ...
  it("renders nothing when closed", () => {
    render(
      <ShareModal isOpen={false} onClose={mockClose} boardId={testBoardId} />
    );
  ...
  it("renders modal when open", () => {
    render(
      <ShareModal isOpen={true} onClose={mockClose} boardId={testBoardId} />
    );
  ...
  it("displays correct board URL", () => {
    ...
    expect(inputElement.value).toBe(
      "https://retro-tool.example.com/board/test-board-123"
    );
  });
  ...
  it("shows QR code button by default", () => {
    ...
    expect(
      screen.queryByAltText("QR Code for board link")
  ...
  it("QR code has correct source URL", async () => {
    ...
    const qrCodeImg = screen.getByAltText(
      "QR Code for board link"
    ) as HTMLImageElement;
    const expectedUrl = encodeURIComponent(
      "https://retro-tool.example.com/board/test-board-123"
    );
    expect(qrCodeImg.src).toContain(
      `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${expectedUrl}`
    );
  });
});
