...
describe('presenceService', () => {
  ...
  const mockBoardRef = { key: 'boards/test-board-id/participants/test-user-id' };
  ...
  describe('setupPresence', () => {
    it('should set up presence tracking for a user', () => {
      ...
      expect(set).toHaveBeenCalledWith(mockBoardRef, expect.objectContaining({
        id: mockUserId,
    ...
    it('should return empty cleanup function when no user is authenticated', () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith('Cannot setup presence without an authenticated user');
